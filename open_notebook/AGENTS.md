...
- Auth is per-user cookie sessions (ADR-010): `api/routers/auth.py` (login/logout/me) + `api/auth_service.py` (token issue/resolve, rate limiting); `api/access.py` `get_current_user` is the dependency seam. Only the analytics router consumes it today — router-level enforcement everywhere is T5. `OPEN_NOTEBOOK_PASSWORD` no longer exists.
