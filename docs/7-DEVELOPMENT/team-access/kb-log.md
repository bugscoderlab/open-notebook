...
| `KB/open-notebook/Auth-probe flip-flop loop.md` | Layout gating on `isCheckingAuth` + every `useAuth()` mount re-probing = infinite unmount/remount `/auth/me` loop; short-circuit loaded identity + first-check-only gating |
...
| `KB/open-notebook/SurrealDB DEFINE FIELD does not overwrite.md` | v2: `DEFINE FIELD IF NOT EXISTS` skips existing fields and plain DEFINE errors — redefinition migrations need REMOVE + DEFINE; a str return from `conn.query` is an error string, not a record |
